...
def amplifer(in_pipe, out_pipe, args):
...
outputs = []
for perm in perms:
    a0 = threading.Thread(target=amplifer, args=(pipes[0], pipes[1], [0,perm[0]],))
    threads = [a0]
    for index in range(1,5):
        threads.append(
          threading.Thread(
            target=amplifer, args=(pipes[index], pipes[(index+1) % 5], [perm[index]],)
            )
          )
